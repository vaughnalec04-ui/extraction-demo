...
The accuracy bar is met. The cost bar is met. The third one, knowing when it's wrong, isn't. That's the whole talk, and I'll show you the numbers behind each of those.
...
That split is the main decision. Extraction is one program. Scoring is another. Once the responses are cached, every threshold sweep is free, tuning is repeatable, and the demo you're about to see runs offline and gives the same numbers every time. The cache key is the model, the prompt version, the schema, and the bytes of the image, and both programs recompute it when they read. Regenerate a page and try to score it against an old response, and it refuses. That failure wouldn't crash. It'd hand you plausible wrong numbers. So it gets a check.

Four configurations come out of the same cached calls. One reader on its own. The second reader on its own. A cascade, where a page goes to the second reader when the first one's confidence is low. And double-keying. Both readers read every page, and any field they disagree on goes to a human. That idea comes straight from how claims shops already work. Two people key the same document and a supervisor settles the differences. I automated the control they already trust.

[Scroll to 1.3, the trade-offs table.]

Now the free tier. My first design escalated to a Flash-class model. Every Flash model I pushed toward evaluation volume stopped after about twenty requests and started returning four-twenty-nines. So the second reader became an independent lite-class model instead. For a disagreement signal, independence matters more than raw capability, so the evaluation survived. The cost is that the cascade has nothing stronger to escalate to. You'll see that in the results.

## 4. Live run (3:40 to 4:50)
...
## 5. Two documents (4:50 to 6:50)

[Tab 3. Point at the total, then the policy number.]

Two pages tell most of the story. This is a degraded repair estimate. The total is four thousand eight hundred and four dollars. The primary reader read forty-one thousand eight hundred and four. One extra digit on a faded scan. It got the policy number wrong too. Those are its only two mistakes on the whole test split, both on this page, and they came with its two lowest confidences, point nine and point nine five. The second reader got the total right and the policy number wrong in a different way. So under double-keying both fields went to review. On a real claim that's a thirty-seven thousand dollar overpayment that didn't go out.

[Tab 4. Point at Amount Due.]

This one's a utility bill. It's in the corpus because it isn't a claim, so the right answer for every field is "nothing here." The primary said nothing here on all six. The second reader, working alone, invented four values. The account number became a policy number. The billing period became a date of service. The utility became the provider. And this amount due, a hundred sixty-eight dollars forty-four, became a claim total. All at confidence one point oh. Under double-keying none of that got out, because the readers disagreed. That's the job of the second reader. On this sample the primary happened to be right, so those flags cost a review each. Flip the roles and they're saved payouts.

## 6. The rule in code (6:50 to 7:40)
...
## 7. What we found (7:40 to 9:10)

[Terminal. Scroll to the primary block and the line that says DEGENERATE. Then Tab 1, the Cascade paragraph.]

Okay, so what did we actually find? Three things, and I want to be straight about all of them.

First, self-reported confidence. The second reader said one point oh on all two hundred forty fields. The primary said one point oh on two hundred fourteen. The harness flags the calibration curve as degenerate, meaning there's only one bin with enough points in it to count. You can't build a gate on a constant.
...
Third, the abstention signal itself. Eight flags in two hundred forty fields. Two were real errors. That's twenty-five percent precision, and the interval runs from seven to fifty-nine. So I can't tell you yet whether double-keying pays for itself. I can tell you exactly what would settle it. About five hundred of Meridian's real documents, weighted toward the exceptions their contractors already flag, through this harness unchanged. The pass-fail gates are written down in the briefing.

## 8. From an eval to a benchmark (9:10 to 10:20)
...
## 9. Friction, for the Gemini teams (10:20 to 11:30)

[Tab 6. It opens at F-007. Read the counts, then scroll down to F-010.]

Two things surprised me enough to write up for the Gemini teams, and two smaller ones. The log has eleven entries, written as they happened, with the error text as it appeared.

F-007. Self-reported confidence is near-constant, and log probabilities aren't available for structured output. For any partner whose requirement is "know when you're unsure," a calibrated confidence output is the missing piece. Right now a second model fills that gap, at extra cost.

F-010. The free tier stopped every Flash model I pushed toward evaluation volume after about twenty requests. The whole evaluation costs about a dollar at list price. That's the tier a partner uses to decide whether Gemini fits, and it can't support an evaluation. A per-project evaluation allowance would fix it.

The two smaller ones. The same four-twenty-nine message covers a rate limit and an exhausted entitlement, so a client can't tell whether to wait or stop. And batch create returns a failed precondition without saying which one, so I never got to measure the half-price batch mode.

## 10. Close (11:30 to 12:00)

[Tab 1, section 4. Back to the camera for the last two sentences.]

So here's where it lands. The accuracy bar's met, with no margin. The cost bar's met with room to spare. The bar that protects payouts, knowing when it's wrong, isn't met yet, and the harness is what tells you that instead of hiding it. It reproduces from a clean clone in one command. Feed it five hundred real documents and it'll tell you whether the second reader earns its keep. That's the next step, and everything you saw is in the repo. Thanks.
